...
// Feature: Review crawler service with scheduled polling of external platforms
...
export default function CfReviewCrawlerServiceWithScheduledPollingOf() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-review-crawler-service-with-scheduled-polling-of-external/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-review-crawler-service-with-scheduled-polling-of-external', project: 'AISocialProofGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-review-crawler-service-with-scheduled-polling-of-external/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Review crawler service with scheduled polling of external platforms</h1>
