...
// Feature: Competitive testimonial analysis via RAG over competitor reviews to highlight differentiation
...
export default function CfCompetitiveTestimonialAnalysisViaRagOverCompetitor() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-competitive-testimonial-analysis-via-rag-over-competitor-reviews/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-competitive-testimonial-analysis-via-rag-over-competitor-reviews', project: 'AISocialProofGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-competitive-testimonial-analysis-via-rag-over-competitor-reviews/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Competitive testimonial analysis via RAG over competitor reviews to highlight differentiation</h1>
