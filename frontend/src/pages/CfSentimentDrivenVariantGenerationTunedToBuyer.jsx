...
// Feature: Sentiment-driven variant generation tuned to buyer personas (price-conscious, feature-seekers, enterprise)
...
export default function CfSentimentDrivenVariantGenerationTunedToBuyer() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-sentiment-driven-variant-generation-tuned-to-buyer-personas-price-conscious/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-sentiment-driven-variant-generation-tuned-to-buyer-personas-price-conscious', project: 'AISocialProofGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-sentiment-driven-variant-generation-tuned-to-buyer-personas-price-conscious/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Sentiment-driven variant generation tuned to buyer personas (price-conscious, feature-seekers, enterprise)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AISocialProofGenerator</p>
      </div>
