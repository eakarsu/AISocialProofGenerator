...
// Feature: Multi-modal proof widgets combining video + audio + text with auto-transcription
...
export default function CfMultiModalProofWidgetsCombiningVideoAudio() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-modal-proof-widgets-combining-video-audio-text-with/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-multi-modal-proof-widgets-combining-video-audio-text-with', project: 'AISocialProofGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-modal-proof-widgets-combining-video-audio-text-with/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Multi-modal proof widgets combining video + audio + text with auto-transcription</h1>
