...
export default function GapNoIntegrationsWithTrustpilotG2CapterraReview() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-integrations-with-trustpilot-g2-capterra-review-platforms/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-integrations-with-trustpilot-g2-capterra-review-platforms', project: 'AISocialProofGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-integrations-with-trustpilot-g2-capterra-review-platforms/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No integrations with Trustpilot, G2, Capterra review platforms (API sync)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AISocialProofGenerator</p>
      </div>
