...
// Feature: One-click marketplace publishing to Capterra, G2, ProductHunt with pre-formatted fields
...
export default function CfOneClickMarketplacePublishingToCapterraG2() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-one-click-marketplace-publishing-to-capterra-g2-producthunt-with/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-one-click-marketplace-publishing-to-capterra-g2-producthunt-with', project: 'AISocialProofGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-one-click-marketplace-publishing-to-capterra-g2-producthunt-with/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">One-click marketplace publishing to Capterra, G2, ProductHunt with pre-formatted fields</h1>
