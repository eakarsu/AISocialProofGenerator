...
export default function GapNoWebhooksNotificationsSystemForNewReview() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-webhooks-notifications-system-for-new-review-or/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-webhooks-notifications-system-for-new-review-or', project: 'AISocialProofGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-webhooks-notifications-system-for-new-review-or/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No webhooks/notifications system for new review or testimonial events</h1>
