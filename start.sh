...
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)";set -a;[[ ! -f "$PROJECT_DIR/.env" ]]||source "$PROJECT_DIR/.env";set +a;BACKEND_PORT="${BACKEND_PORT:-${PORT:-5001}}";FRONTEND_PORT="${FRONTEND_PORT:-3000}";ALLOWED_ORIGINS="${ALLOWED_ORIGINS:-http://127.0.0.1:$FRONTEND_PORT,http://localhost:$FRONTEND_PORT}";export BACKEND_PORT FRONTEND_PORT ALLOWED_ORIGINS
...
if [ "${NODE_ENV:-development}" != production ] && [ "${ENABLE_DEMO_CREDENTIAL_AUTOFILL:-true}" = true ]; then psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f "$PROJECT_DIR/backend/migrations/001_governed_social_proof.sql" >/dev/null; node "$PROJECT_DIR/backend/seeds/provision-demo-credentials.js"; fi
